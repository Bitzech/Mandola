...
export default function SellerChangePassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Seller password changed.");
      setSuccess(true);
      setForm({ current: "", newPass: "", confirm: "" });
      setTimeout(() => setSuccess(false), 4000);
    } catch (err: any) {
      const msg = extractErrorMessage(err, "Failed to change seller password.");
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      ...
      <div className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="bg-[#ffffff] border border-[#ececec] p-6 space-y-5">
