  ...
  return (
    <div>
      ...
      <div className="max-w-md">
        {error   && <div className="mb-5 bg-red-50 border border-red-200 text-red-600 px-4 py-3 text-xs tracking-wide">{error}</div>}
        {success && <div className="mb-5 bg-green-50 border border-green-200 text-green-700 px-4 py-3 text-xs tracking-wide">Password updated successfully.</div>}
            ...
            <div key={key}>
              ...
              <div className="relative">
                <input
                  ...
                  onChange={set(key)}
                  placeholder="••••••••"
                  className="w-full border border-[#ececec] px-4 py-3 pr-14 text-sm text-[#1a1a1a] placeholder-[#c0c0c0] focus:outline-none focus:border-[#d4145a] bg-white"
                />
                <button type="button" onClick={() => setShow(prev => ({ ...prev, [key]: !prev[key] }))} className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] tracking-wide text-[#6e6e6e] hover:text-[#d4145a]">
                  {show[key] ? "Hide" : "Show"}
                </button>
              </div>
            </div>
